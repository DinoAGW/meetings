...
public class LinkDownload {
	...
	public static void linkDownload(Kongress it, String mainPath, String protokoll, String hostname)
			throws IOException {
		...
		BufferedReader br = new BufferedReader(new FileReader(new File(kongressDir + "content" + fs + "checksum.md5")));
		String line;
		...
		br.close();
		for (int i = 0; i < owner_links.size(); i++) {
			br = new BufferedReader(new FileReader(new File(kongressDir + i + fs + "content" + fs + "checksum.md5")));
			while ((line = br.readLine()) != null) {
				lines.add(line);
			}
			br.close();
		...
		File cssFile = new File(kongressDir + "merge" + fs + "content" + fs + "www.egms.de" + fs + "static" + fs + "css"
				+ fs + "gms-framework.css");
		...
		for (Element session : sessionlist) {
			...
			for (Element abstractElement : abstractlist) {
				System.out.println(abstractElement.attr("href"));
			}
		}
	}
	...
	public static void linkDownload(String mainPath, String protokoll, String hostname)
		...
		while (resultSet.next()) {
			...
			linkDownload(it, mainPath, protokoll, hostname);

			// Den Fortschritt in der Datenbank vermerken
			// int updated = sqlManager.executeUpdate("UPDATE ueberordnungen SET Status = 30
			// WHERE ID = '" + it.kurzID + "';");
