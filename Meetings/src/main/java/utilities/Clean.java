...
public class Clean {
	...
	public static void clean() {
		...
		try {
			...
			stmt.executeUpdate("CREATE TABLE ueberordnungen ( ID VARCHAR(20), URL VARCHAR (200), LANG VARCHAR(5), Status INT );");
			stmt.executeUpdate("DROP TABLE IF EXISTS abstracts;");
			stmt.executeUpdate("CREATE TABLE abstracts ( Ue_ID VARCHAR(20), Ab_ID VARCHAR(20), URL VARCHAR (200), LANG VARCHAR(5), Status INT );");
